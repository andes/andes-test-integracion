...
context('auditoria', () => {
    ...
    const notaError = 'se deben cambiar nombres del paciente por Juan Auditado';
    before(() => {
        cy.seed();
    ...
    it('Reportar error desde mpi y verificar que aparezca en el listado y detalle', () => {
        ...
        cy.plexTextArea('name="motivo"', notaError);
    ...
    it('Modificar nombre de un paciente reportado con error', () => {
        ...
        cy.wait('@getReportados').then(xhr => {
            expect(xhr.status).to.be.eq(200);
            //expect(xhr.responseBody).to.have.length(cantReportes - 1);
        });
