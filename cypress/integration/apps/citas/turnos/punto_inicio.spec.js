context('punto de inicio', () => {
    ...
    it('Buscar paciente inexistente', () => {
        cy.get('plex-text input[type=text]').first().type('12362920').should('have.value', '12362920');
    ...
    it('dar turno', () => { 
        ...
        cy.get('plex-button[title="Dar Turno"]').click({force: true});        
    ...
    it('generar solicitud', () => {
        ...
        cy.get('plex-button[title="Solicitudes"]').click({force: true});        
        cy.wait('@generarSolicitudPaciente').then((xhr) => {
            expect(xhr.status).to.be.eq(200);
        });
        cy.get('lista-solicitud-turno-ventanilla plex-button[label="Cargar Solicitud nueva"]').click({force: true});
    });

    it('activar app mobile', () => {
        cy.route('GET', '**api/core/mpi/modules/mobileApp/check/**').as('clickActivarApp');
        ...
        cy.get('paciente-listado').find('td').contains('38906734').click();
        cy.get('plex-button[title="Activar App"]').click({force: true});
        cy.get('plex-text[placeholder="Celular"] input[type=text]').first().clear();
        cy.get('plex-text[placeholder="e-mail"] input[type=text]').first().clear();
        cy.get('plex-text[placeholder="e-mail"] input[type=text]').first().type('prueba@prueba.com').should('have.value', 'prueba@prueba.com');
        cy.get('plex-text[placeholder="Celular"] input[type=text]').first().type('2995290357').should('have.value', '2995290357');
        cy.get('plex-button[label="Activar App Mobile"]').click({force: true});
        cy.swal('confirm')
    })
})
