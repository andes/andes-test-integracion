...
context('RUP - Punto de inicio', () => {
    ...
    it('Iniciar y validar prestación niño sano 1 mes- Fuera de agenda', () => {
        cy.goto('/rup', token);
    ...
    it('Iniciar y validar prestación niño sano 2.5 años - Fuera de agenda', () => {
        cy.goto('/rup', token);
        ...
        cy.wait(3000); // le da tiempo para que cargue el sidebar
        cy.get('plex-text input').first().type('2 años y medio');
        cy.wait(3000);

        cy.get('.mdi-plus').first().click();

        cy.get('rup-peso input').eq(0).type('7');
        cy.get('rup-talla input').eq(0).type('95');
        cy.get('rup-percentilo-talla input').eq(0).type('59');
        cy.get('rup-perimetrocefalico input').eq(0).type('26');
        cy.get('rup-percentiloperimetrocefalico input').eq(0).type('64');
        cy.get('rup-percentilo-imc input').eq(0).type('85');
        cy.get('rup-desarrollo-psicomotor plex-bool').eq(1).click();
        cy.get('rup-lactancia plex-bool').eq(2).click();
        cy.get('span').contains('Guardar consulta de niño sano').click({
            force: true
        });
        cy.wait('@patch').then((xhr) => {
    ...
    it('Iniciar y validar prestación niño sano 5 años - Fuera de agenda', () => {
        cy.goto('/rup', token);
        ...
        cy.wait(3000); // le da tiempo para que cargue el sidebar
        cy.get('plex-text input').first().type('5 años');
        cy.wait(3000);

        cy.get('.mdi-plus').first().click();

        cy.get('rup-peso input').eq(0).type('57');
        cy.get('rup-talla input').eq(0).type('100');
        cy.get('rup-percentilo-talla input').eq(0).type('65');
        cy.get('rup-percentilo-imc input').eq(0).type('76');
        cy.get('rup-tension-sistolica input').eq(0).type('75');
        cy.get('rup-tension-diastolica input').eq(0).type('70');
        cy.get('rup-percentilo-de-tension-arterial input').eq(0).type('68');
        cy.get('rup-desarrollo-psicomotor plex-bool').eq(1).click();
        cy.get('span').contains('Guardar consulta de niño sano').click({
            force: true
        });
