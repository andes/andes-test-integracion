...
context('Aliasing', () => {
    ...
    it.skip('crear solicitud de salida', () => { // TODO: molesta el wizard
        ...
        cy.get('plex-select[label="Tipo de Prestación Solicitada"]').children().children('.selectize-control').click({ force: true })
        .find('.option[data-value="59ee2d9bf00c415246fd3d6a"]').click({ force: true })

        cy.get('plex-select[label="Organización origen"]').children().children().children('.selectize-input').click({ force: true }).get('.option[data-value="57e9670e52df311059bc8964"]').click({ force: true })
        cy.get('plex-select[label="Tipos de Prestación Origen"]').children().children().children('.selectize-input').click({ force: true }).get('.option[data-value="391000013108"]').click({ force: true })
