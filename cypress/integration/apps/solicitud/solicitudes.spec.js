...
context('TOP', () => {
    ...
    it('crear solicitud de entrada y verificar filtros', () => {
        ...
        cy.get('plex-select[label="Tipo de Prestación Solicitada"]').children().children().children('.selectize-input').click({
            force: true
        }).get('.option[data-value="59ee2d9bf00c415246fd3d6d"]').click({
            force: true
        });
        ...
        cy.get('plex-select[label="Organización origen"]').children().children().children('.selectize-input').click({
            force: true
        }).get('.option[data-value="57fcf038326e73143fb48dac"]').click({
            force: true
        });
        cy.get('plex-select[label="Tipos de Prestación Origen"]').children().children().children('.selectize-input').click({
            force: true
        }).get('.option[data-value="401000013105"]').click({
            force: true
        });
        cy.get('plex-select[name="profesionalOrigen"] input').type('cortes jazmin');
        cy.wait('@getProfesional').then((xhr) => {
            expect(xhr.status).to.be.eq(200);
        });
        cy.get('plex-select[name="profesionalOrigen"] input').type('{enter}');
        cy.get('plex-select[name="profesional"] input').type('natalia huenchuman');
        ...
        cy.get('plex-select[name="organizacion"] input').type('hospital dr. horacio heller').get('.option[data-value="57fcf038326e73143fb48dac"]').click({
            force: true
        });
        cy.get('plex-select[name="prestacionDestino"] input').type('consulta de neurología').get('.option[data-value="59ee2d9bf00c415246fd3d6d"]').click({
            force: true
        });
        cy.get('plex-select[name="estado"] input').type('auditoria').get('.option[data-value="auditoria"]').click({
            force: true
        });
