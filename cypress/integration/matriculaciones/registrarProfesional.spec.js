...
context('Aliasing', () => {
    ...
    it('cargar profesional en matriculaciones', () => {
        cy.server();

        //DATOS GENERALES 

        cy.get('plex-button[label="Nuevo Profesional"]').click();
        cy.get('plex-text[label="apellido"] input').type('Palavecino');
        cy.get('plex-text[label="Nombre"] input').type('Mariano');
        cy.get('plex-select[label="Sexo"] input').type('masculino{enter}');
        cy.get('plex-select[label="Nacionalidad"] input').type('argentina{enter}');
        cy.get('plex-datetime[label="Fecha de Nacimiento"] input').type('18/01/1991');
        cy.get('plex-text[label="Lugar de Nacimiento"] input').type('San Carlos de Bariloche');
        cy.get('plex-int[label="C.U.I.T. / C.U.I.L."] input').type('2a3b3f5d5e9r3y5d4f6w9').should('have.value', '23355935469');
        cy.get('plex-select[label="Tipo de Documento"] input').type('dni{enter}');
        cy.get('plex-int[label="Nº de Documento"] input').type('3a5c5d9w3r5e4q6').should('have.value', '35593546');
        ...
        //Domicilio Real
        cy.get('plex-select[name="provinciaReal"] input').type('Neuquen{enter}');
        cy.get('plex-select[name="localidadReal"] input').type('Neuquen{enter}');
        cy.get('plex-text[name="direccionReal"] input').type('Dewey 521');
        ...
        cy.get('plex-select[name="formacionPosgrado"] input').type('Kinesiólogo{enter}');
        cy.get('plex-text[name="titulo"] input').type('Licenciatura de Kinesiologia');
        cy.get('plex-datetime[name="fechaEgreso"] input').type('05/11/2007');
        ...
        cy.get('plex-select[name="entidadFormadora"] input').type('Escuela superior de salud publica{enter}');
        ...
        cy.get('plex-text[label="Documento"] input').type('35593546');
        cy.wait(4000);
